...
describe('convert function', () => {
  ...
  it('converts byte arrays to utf8', () => {
    ...
    const element: SchemaElement = { name, converted_type: ConvertedType.UTF8, logical_type: undefined }
    expect(convert(data, { element, parsers })).toEqual(['foo', 'bar'])
  })

  it('converts byte arrays to utf8 default true', () => {
    const data = [new TextEncoder().encode('foo'), new TextEncoder().encode('bar')]
    const element: SchemaElement = { name, type: ParquetType.BYTE_ARRAY, logical_type: undefined }
    expect(convert(data, { element, parsers })).toEqual(['foo', 'bar'])
  })
  ...
  it('converts numbers to DECIMAL', () => {
    const data = [100, 200]
    const element: SchemaElement = { name, converted_type: ConvertedType.DECIMAL, logical_type: undefined }
    expect(convert(data, { element, parsers })).toEqual([100, 200])
  })

  it('converts numbers to DECIMAL with scale', () => {
    const data = [100, 200]
    const element: SchemaElement = { name, converted_type: ConvertedType.DECIMAL, scale: 2, logical_type: undefined }
  ...
  it('converts byte arrays to DECIMAL', () => {
    ...
    const element: SchemaElement = { name, converted_type: ConvertedType.DECIMAL, scale: 0, logical_type: undefined }
    expect(convert(data, { element, parsers })).toEqual([100, 200])
  })

  it('converts byte array from issue #59 to DECIMAL', () => {
    const data = [new Uint8Array([18, 83, 137, 151, 156, 0])]
    const element: SchemaElement = { name, converted_type: ConvertedType.DECIMAL, scale: 10, precision: 14, logical_type: undefined }
    expect(convert(data, { element, parsers })).toEqual([2015])
  })

  it('converts epoch time to DATE', () => {
    const data = [1, 2] // days since epoch
    const element: SchemaElement = { name, converted_type: ConvertedType.DATE, logical_type: undefined }
  ...
  it('converts INT96 to DATE', () => {
    ...
    const element: SchemaElement = { name, type: ParquetType.INT96, logical_type: undefined }
  ...
  it('converts epoch time to TIMESTAMP_MILLIS', () => {
    ...
    const element: SchemaElement = { name, converted_type: ConvertedType.TIMESTAMP_MILLIS, logical_type: undefined }
  ...
  it('converts epoch time to TIMESTAMP_MICROS', () => {
    ...
    const element: SchemaElement = { name, converted_type: ConvertedType.TIMESTAMP_MICROS, logical_type: undefined }
    expect(convert(data, { element, parsers })).toEqual([
      new Date('2024-05-23T23:28:20.000Z'), new Date('2024-05-23T23:30:00.000Z'),
    ])
  })
  ...
  it('parses strings to JSON', () => {
    ...
    const element: SchemaElement = { name, converted_type: ConvertedType.JSON, logical_type: undefined }
    expect(convert(data, { element, parsers })).toEqual([{ key: true }, { quay: 314 }])
  })

  it('converts uint64', () => {
    const data = [100n, -100n]
    const element: SchemaElement = { name, converted_type: ConvertedType.UINT_64, logical_type: undefined }
  ...
  it('throws error for BSON conversion', () => {
    const data = [{}]
    const element: SchemaElement = { name, converted_type: ConvertedType.BSON, logical_type: undefined }
  ...
  it('throws error for INTERVAL conversion', () => {
    const data = [{}]
    const element: SchemaElement = { name, converted_type: ConvertedType.INTERVAL, logical_type: undefined }
    expect(() => convert(data, { element, parsers }))
